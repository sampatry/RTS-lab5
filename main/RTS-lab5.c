...
void app_main(void)
{
    data_Queue = xQueueCreate(5, sizeof(int)); // Create queue to store up to 5 values of ADC data

    i2c_init(data_Queue); // Setup I2C master, and data queue
    adc_init(data_Queue); // Setup ADC GPIO, voltage range, bits and data queue
    xTaskCreate(I2C_send, "master-send-task", 1024, NULL, 1, NULL);
